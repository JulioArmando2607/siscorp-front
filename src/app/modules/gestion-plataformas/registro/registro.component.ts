...
export class RegistroComponent implements OnInit {
  ...
  async save() {
    ...
    if (oRespL.data.codResultado == 200) {
      this.dataModal(oRespL.data.codResultado, "Exito!", oRespL.data.msgResultado)
      this.matDialogRef.close('success'); // Enviar un resultado al modal padre
    } else if (oRespL.data.codResultado == 400) {
      this.dataModal(oRespL.data.codResultado, "Error!", oRespL.data.msgResultado)
    } else{
      this.dataModal(oRespL.data.codResultado, "Error!", oRespL.data.msgResultado)
    }
  }
  ...
  removeFile() {
    ...
    this.FormRegistro.get('departamento')?.setValidators([Validators.required]);
  ...
  async getDetalleCentrosPoblados(ubigeo) {
    ...
    this.FormRegistro.get("departamento").setValue(response.departamentoUbigeo)
    ...
    this.FormRegistro.get("centroPoblado").setValue(response.centroPobladoUbigeo)
  ...
  onBuscar(event: any) {
    ...
    if (valor.length > 9) { // Evita búsquedas con pocas letras
